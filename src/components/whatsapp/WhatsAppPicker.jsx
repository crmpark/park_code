import { useState, useRef, useEffect } from 'react'
import { MessageCircle, ChevronDown, ExternalLink } from 'lucide-react'
import { supabase } from '../../lib/supabase'
import { whatsappLink } from '../../lib/utils'
import { getStageMessages, getDefaultStageMessage } from '../../lib/whatsappMessages'

function replaceVars(body, name, company, advisor) {
  const first = name?.split(' ')[0] || ''
  return (body || '')
    .replace(/\{\{nombre\}\}/gi, first)
    .replace(/\{\{empresa\}\}/gi, company || '')
    .replace(/\{\{asesor\}\}/gi, advisor || 'Tu asesor')
}

export function WhatsAppPicker({ phone, prospectName, companyName, pipelineStage, advisorName, size = 'md' }) {
  const [open, setOpen] = useState(false)
  const [dbTemplates, setDbTemplates] = useState(null)
  const ref = useRef(null)

  // Cargar plantillas de BD para esta etapa
  useEffect(() => {
    if (!pipelineStage) return
    supabase
      .from('whatsapp_templates')
      .select('*')
      .eq('stage_slug', pipelineStage)
      .eq('active', true)
      .order('created_at')
      .then(({ data }) => {
        setDbTemplates(data?.length ? data : null)
      })
      .catch(() => setDbTemplates(null))
  }, [pipelineStage])

  // Cerrar al hacer click fuera
  useEffect(() => {
    function onClickOut(e) {
      if (ref.current && !ref.current.contains(e.target)) setOpen(false)
    }
    if (open) document.addEventListener('mousedown', onClickOut)
    return () => document.removeEventListener('mousedown', onClickOut)
  }, [open])

  if (!phone) return null

  // Unificar: BD > hardcoded
  const messages = dbTemplates
    ? dbTemplates.map((t) => ({
        label: t.name,
        getText: () => replaceVars(t.message_body, prospectName, companyName, advisorName),
      }))
    : getStageMessages(pipelineStage).map((msg) => ({
        label: msg.label,
        getText: () => msg.getMessage(prospectName?.split(' ')[0] || '', companyName),
      }))

  const btnClass = `inline-flex items-center gap-2 bg-green-500 text-white rounded-lg font-medium hover:bg-green-600 transition-colors duration-200 ${
    size === 'sm' ? 'px-2.5 py-1.5 text-xs' : 'px-3 py-2 text-sm'
  }`
  const iconSize = size === 'sm' ? 14 : 16

  // Solo un mensaje — botón directo
  if (messages.length <= 1) {
    const text = messages.length
      ? messages[0].getText()
      : getDefaultStageMessage(pipelineStage, prospectName, companyName)
    return (
      <a href={whatsappLink(phone, text)} target="_blank" rel="noopener noreferrer" className={btnClass}>
        <MessageCircle size={iconSize} />
        WhatsApp
      </a>
    )
  }

  // Múltiples mensajes — dropdown
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={(e) => { e.stopPropagation(); setOpen(!open) }}
        className={btnClass}
      >
        <MessageCircle size={iconSize} />
        WhatsApp
        <ChevronDown size={12} className={`transition-transform duration-200 ${open ? 'rotate-180' : ''}`} />
      </button>

      {open && (
        <div className="absolute right-0 top-full mt-1 bg-white rounded-xl shadow-xl border border-gray-100 py-2 w-72 z-30">
          <p className="px-3 pb-2 text-[10px] uppercase tracking-wider text-gray-400 font-semibold border-b border-gray-100 mb-1">
            Selecciona un mensaje
          </p>
          {messages.map((msg, i) => {
            const text = msg.getText()
            return (
              <a
                key={i}
                href={whatsappLink(phone, text)}
                target="_blank"
                rel="noopener noreferrer"
                onClick={() => setOpen(false)}
                className="flex items-start gap-2.5 px-3 py-2.5 hover:bg-green-50 transition-colors duration-150 group"
              >
                <div className="w-7 h-7 rounded-full bg-green-100 flex items-center justify-center shrink-0 mt-0.5">
                  <MessageCircle size={13} className="text-green-600" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold text-gray-800 mb-0.5 flex items-center gap-1">
                    {msg.label}
                    <ExternalLink size={9} className="text-gray-300 group-hover:text-green-500 transition-colors" />
                  </p>
                  <p className="text-[10px] text-gray-500 line-clamp-2 leading-relaxed">{text}</p>
                </div>
              </a>
            )
          })}
        </div>
      )}
    </div>
  )
}
